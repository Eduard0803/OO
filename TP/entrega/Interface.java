...
class Interface{
    ...
    public Estacionamento getEstacionamento(){return p;} // metodo get para o objeto do estacionamento
    ...
    private void setAcessoPage(){ // Define a janela de Acessos
        ...
        botao.addActionListener(e -> setAcesso(p)); // executa o metodo 'setAcesso' quando clicar no botão 'Inserir Acesso'
        JButton close = new JButton("Fechar"); // define o botão para fechar a janela
        close.addActionListener(e -> closeAction(acessoFrame));
        acessPanel.setLayout(new GridLayout(11, 4)); // define o tamanho do painel
	...
	private void setEstacionamentoPage(){ // define a janela de cadastro de Estacionamentos
        ...
        button.addActionListener(e -> p = setEstacionamento()); // executa o metodo 'setEstacionamento' quando clicar no botão 'Cadastrar Acessos'
        JButton close = new JButton("Fechar"); // define o botão para fechar a janela
    ...
    private void setAcesso(Estacionamento p){ // metodo para definir um novo Acesso ao Estacionamento
        ...
        try{ // tratamento da exceção do tipo 'ValorAcessoInvalidoException'
            p.inserirAcesso(new Acesso(placa, new Duracao("01/01/2000", horaEntrada, "01/01/2000", horaSaida), ta));
        }catch(ValorAcessoInvalidoException e){
            closeAction(acessoFrame);
            JOptionPane.showMessageDialog(null, e.getMessage());
            exibirEstacionamentoPage();
        }
        exibirValor(p.calcValor(placa)); // exibe o valor à ser pago pelo Acesso
